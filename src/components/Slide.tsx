...
export default function Slide({ image, label, title, subtitle, currentIndex }: SlideProps) {
    const variantKey = labelToVariantKeyMap[label as keyof typeof labelToVariantKeyMap] as SlideVariantKeys;
  
    const currentVariant = slideVariants[variantKey] ?? slideVariants['documentScanner'];
  
    return (
      <div key={currentIndex} className="w-full max-w-[1440px] mx-auto flex flex-col-reverse items-center gap-10 px-4 md:flex-row md:items-start md:justify-between md:gap-[76px] md:px-0">
